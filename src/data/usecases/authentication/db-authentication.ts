import { AddAccountsParams, Authentication, AuthResponse, Encrypter, FindAccountByEmailRepository, HashComparer } from './db-authentication-protocols'
  ...
  async auth(authentication: AddAccountsParams): Promise<AuthResponse> {
    ...
    if (account) {
      ...
      if (isValid) {
        ...
        return { user: { name, email, id }, accessToken }
      }
    }
    return null
  }
